...
class OccupancyGrid():
    ...
    def update_cell_with_meas_logodds(self, x_idx, y_idx, l):
        ...
        self.grid[y_idx,x_idx] += (l - np.log(self.prior/(1-self.prior)))
...
class OccupancyGridMap():
    ...
    def laser_range_inverse_sensor_model(self, x_idx, y_idx, x_t, z_t):
        ...
        # Find the distance between the sensor and cell under consideration
        x,y = self.ogrid.get_cell_center(x_idx,y_idx)

        dist = np.sqrt((x - x_t.x)**2 + (y - x_t.y)**2)
        r = self.ogrid.resolution
    ...
    def find_cells_to_update_for_ray(self, x_t, z_theta_t, max_range):
        ...
        global_th_rads = (z_theta_t+x_t.theta) * np.pi / 180
        x_pos, y_pos = x_t.x, x_t.y

        res = self.ogrid.resolution
        step_res = 1/5.0 * res

        x_inc = np.cos(global_th_rads) * step_res
        y_inc = np.sin(global_th_rads) * step_res

        cells_to_update = set()

        for _ in range(0, int(max_range / step_res)):
          x_pos += x_inc
          y_pos += y_inc
          new_cell_pos = tuple(self.ogrid.get_cell_index(x_pos, y_pos))
          cells_to_update.add(new_cell_pos)

        # returns as list of tuples with (x, y)
        # return 0
        return list(cells_to_update)
        ####################################
    ...
    def integrate_laser_range_ray(self, x_t, z_theta_t, z_t):
        ...
        #####################################
        # Finish this implemenation!!
        cells_to_update = self.find_cells_to_update_for_ray(x_t, z_theta_t, z_t)

        for cell in cells_to_update:
          x,y = cell
          log_odds = self.laser_range_inverse_sensor_model(x, y, x_t, z_t)

          self.ogrid.update_cell_with_meas_logodds(x, y, log_odds)
        #####################################
...
def main(datafile, plot_live, resolution):
    ...
    for i in tqdm(range(len(x_t))):
        ...
        for j in range(num_lasers):
            ...
            omap.integrate_laser_range_ray(X, angles[j], z_t[i,j])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--datafile", type=str, default="lab2-occupancy-grid-mapping/data/rooms-dataset.npz", help="Location of data. Defaults to data/rooms-dataset.npz")
