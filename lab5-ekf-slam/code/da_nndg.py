...
botThresh = 1 # TODO: Tune your threshold
# Higher than this is a new landmark
topThresh = 100 # TODO: Tune your threshold
...
def associateData(measurements, R, mu, Sigma, landmarkSignatures = [], shortCircuitThresh = 40.0**2):
    ...
    # If there are previous landmarks
        ...
        for z in measurements:   # For each measurement
            ...
            # Associate the measurement to the landmark that gives the smallest K if it is less than the threshold
            landmark_minK_ind = K.index(min(K))
            if K[landmark_minK_ind] <= botThresh:
                # Association
            ...
            # if the minimum Mahalonobis distance is greater than the threshold, create a new landmark
            elif K[landmark_minK_ind] >= topThresh: 
                association.append(-1) # We say the association is "-1" to tell future code to add new landmarks
                if len(innovation) < 1:
                    innovation = np.array([0, 0])
                    H = np.zeros((2, len(mu)))
