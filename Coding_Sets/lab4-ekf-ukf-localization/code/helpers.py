#!/usr/bin/env python3
"""Module defining several "helper" or "utility" functions useful for the landmark/localization lab.
...
"""
import matplotlib
# matplotlib.use("Qt5Agg") # Might need this on mac if live plotting doesn't work.  Actually dont' know if we're using live plotting for this lab.
...
def plotRobot(pose=[0, 0, 0], color="red", fillColor=None):
...
def plotField(detectedMarker=None):
    """This will plot the field at a given state, pulling from the fieldSettings imported from fieldSettings.py

    Parameters: 
    detectedMarker: If a specific marker index is provided, it will be highlighted in the plot.  Default none.
    ...
    for k in range(field['numMarkers']):
        plotCircle(np.array([field['markerPosX'][k], field['markerPosY'][k]]), 15, 200, 'black', '#00000040' if detectedMarker is not None and k == detectedMarker else '#FFFFFF')
